...
class ComponentRegistry {
  private renderers: Map<string, RenderFn> = new Map();
  ...
  private registerDefaultRenderers() {
    ...
    this.register('card', (s, render) => {
      ...
      if (s.children) {
        const children = this.safeArray<ComponentSpec>(s.children);
        children.forEach(c => kids.push(render(c)));
      }
      if (s.buttons && Array.isArray(s.buttons)) {
        const btnKids: VNode[] = [];
        s.buttons.forEach((btn: any) => btnKids.push(render(btn)));
    ...
    this.register('form', (s, render) => {
      ...
      if (s.fields && Array.isArray(s.fields)) {
        s.fields.forEach((f: ComponentSpec) => kids.push(render(f)));
      }
      if (s.buttons && Array.isArray(s.buttons)) {
        const btnKids: VNode[] = [];
        s.buttons.forEach((btn: any) => btnKids.push(render(btn)));
    ...
    this.register('list', (s, render) => {
      ...
      if (s.items && Array.isArray(s.items) && s.items.length) {
        ...
        const items = s.items.map((item: any, i: number) => {
          const liKids: VNode[] = [];
    ...
    this.register('alert', (s) => {
      ...
      const icons: Record<string, string> = { info: 'ℹ️', success: '✅', warning: '⚠️', danger: '❌', error: '❌' };
      const kids: (VNode | string)[] = [h('span', { class: 'gen-alert-icon' }, s.icon || icons[type] || icons.info)];
  ...
  register(type: string, fn: RenderFn) {
